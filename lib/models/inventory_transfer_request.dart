...
class InventoryTransferRequest {
  ...
  double docTotal;
  String comments;
  List<InventoryTransferRequestLine> lines;
  ...
  InventoryTransferRequest.fromJSON(Map<String, dynamic> jsonMap) {
    try {
      ...
      lines = jsonMap['lines'] != null && (jsonMap['lines'] as List).length > 0
          ? List.from(jsonMap['lines']).map((element) => InventoryTransferRequestLine.fromJSON(element)).toSet().toList()
          : [];
    } catch (e) {
      docEntry = 0;
      docNum = 0;
